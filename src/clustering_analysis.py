...
def load_classification_data(csv_file, level):
    ...
    classification = [bird[index] for bird in bird_info]
    return classification


def assign_classifications(tree, classification):
    for leaf in tree.iter_leaves():
        # In case a leaf name is not found, assign None.
        leaf.add_feature("classification", classification[int(leaf.name.split("_")[0])])
