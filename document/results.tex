...
The visual morphospace exhibit a highly significant phylogenetic signal ($p = 0.001$, 999 permutations). It yields an overall $K_{mult}$ of 0.42. The value $K_{mult} < 1$ indicates that while phylogenetic constraints shape the visual morphospace, the lineage divergence is coupled with substantial trait lability and convergent evolution (homoplasy). The first phylogenetically aligned component (PAC1) alone exhibited extreme phylogenetic conservatism ($K = 2.08, p = 0.001$), demonstrating that the primary axis of visual variation is strongly constrained by shared ancestry, whereas minor axes encapsulate rapid ecological adaptations. 
