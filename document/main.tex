...
\begin{abstract}
\begin{enumerate}
\item The evolution of biological morphology is critical for understanding global biodiversity. However, traditional geometric morphometrics analyses are constrained by the requirement of landmarks. 
\item We propose a novel, scalable deep-learning-based framework that captures phenotypic representations from photos. The hyperspherical embedding with $L_2$ normalisation restricts the learned morphospace to a curved Riemannian manifold (a unit hypersphere). We generalise the Felsenstein's Phylogenetic Independent Contrasts (PIC) method from a Euclidean space to a unit hypersphere.
\item To validate the feasibility of this methodology, we conducted a comprehensive case study on avian visual morphospace evolution, training a ResNet34 framework on over 10,000 bird species (4.8 million images). Multi-dimensional trait vectors were extracted from the model's final fully connected layer to compute hyperspherical disparity, agglomerative links, and ancestral state reconstructions.
\item The validation pipeline successfully encoded phenotypic traits without a priori taxonomic or morphological knowledge. Spherical variance analysis proved that species richness acts as the primary driver of morphospace expansion. Furthermore, our generalised Riemannian PIC successfully recovered a visual "early burst" of intense disparity expansion immediately following the K-Pg mass extinction.
\item The conclusions derived from the validation study are highly congruent with other macroevolutionary studies, demonstrating that our framework provides a novel high-throughput tool. 
\end{enumerate}
\end{abstract}
