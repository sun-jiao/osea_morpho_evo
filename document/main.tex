...
In our study, we attempted to use CNNs as a high-throughput, high-dimensional
morphometry tool capable of capturing holistic visual features, including colour,
...
Then, we conduct 100 times of null simulations in preorder traversals.
...
\begin{enumerate}
    ...
    \end{equation}
    and $\mathbf{v}_{c}$ is the simulated feature vector of the child species.
\end{enumerate}

\input{algorithm-bm-figure}

When the whole traversal is finished, the ASR is conducted based on the simulated
...
are visualised and compared from evolutionary biological perspective.
...
Placeholder (\cref{fig:dtt})

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{dtt_null_test_combined}
    \caption{Grad-CAM reveals that the network's attention is consistently
    focused on birds, ignoring backgrounds and occlusions.}
    \label{fig:dtt}
\end{figure}
